...
$process = Start-Process -FilePath $config.fxServerPath -ArgumentList @('+set', 'txDataPath', $config.txDataPath, '+set', 'serverProfile', $config.txAdminProfile) -WorkingDirectory (Split-Path -Parent $config.fxServerPath) -WindowStyle Hidden -PassThru
