...
RegisterNetEvent('nv_cloth:openShopInteraction', function(label, categories, shopKey)
  if not opened then
    openClothShop(label, categories, shopKey)
  end
end)
        ...
        for _, pos in pairs(shop.coords) do
          ...
          if dist <= 2.0 and not opened then
            ...
            if IsControlJustPressed(0, 38) then -- E
              TriggerEvent('nv_cloth:openShopInteraction', shop.label, shop.categories, shopKey)
            end
          end
        end
      end
