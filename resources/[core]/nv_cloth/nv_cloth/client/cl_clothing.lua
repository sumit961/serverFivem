...
local function normaliseInventoryImagePath(image)
  image = tostring(image or '')
  if image == '' then return nil end
  image = image:gsub('^%s+', ''):gsub('%s+$', '')
  image = image:gsub('\\', '/')

  -- Keep NUI/HTTP URLs untouched. Relative catalog paths are kept relative so
  -- cm-inventory can convert them to cfx-nui paths for the UI.
...
local function buildBuyMetadata(item)
  local ped = PlayerPedId()
  local category = tostring(item.category or item.type or item.categoryType or ''):lower()
  local model = GetEntityModel(ped)
  local gender = model == GetHashKey('mp_f_freemode_01') and 'female' or 'male'
  local image = normaliseInventoryImagePath(item.image or item.icon or item.imagePath or item.image_path or item.imageFile or item.image_file)
...
local function buyCommon(data, payment)
  ...
  for _, item in ipairs(items) do
    ...
    if category ~= 'arms' and category ~= 'gloves' then
      ...
      -- Also mirror important catalog fields at top level for older server bridges.
      -- The server still treats metadata as source of truth, but keeping these values
      -- here prevents image/bag data from being lost by UIs that strip nested fields.
      if item.metadata.image then
        item.image = item.metadata.image
        item.icon = item.metadata.icon
      end
      if item.metadata.description then item.description = item.metadata.description end
      if item.metadata.bagLevel then item.bagLevel = item.metadata.bagLevel end
