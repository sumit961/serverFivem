...
async function mdtTermSetStars(stars) {
  ...
  let reason = '';
  if (next > 0) {
    reason = String((await showPromptOverlay('Wanted Level', 'Enter the reason for assigning this wanted level:')) || '').trim();
...
async function mdtTermToggleWanted() {
  ...
  let reason = '';
  if (settingWanted) {
    reason = String((await showPromptOverlay('Mark Wanted', 'Enter the reason this citizen is wanted:')) || '').trim();
    if (reason.length < 3) return;
  }
