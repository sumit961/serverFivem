...
function renderRanksList(){
  ...
  document.querySelector('#ranksList').innerHTML=(data.ranks||[]).map(r=>{
    ...
    const pills=granted.length?granted.map(k=>`<span class="perm-pill">${esc(permLabels[k]||k)}</span>`).join(''):'<span class="perm-pill perm-pill--empty">No permissions</span>';
...
function renderArmory(){
  const rows=(armoryData?.items||[]).filter(x=>armoryFilter==='all'||x.itemType===armoryFilter);
  document.querySelector('#armoryManage').classList.toggle('hidden',armoryData?.canManage!==true);
  document.querySelector('#armoryGrid').innerHTML=rows.map(item=>{
    const ratio=item.maxStock?item.stock/item.maxStock:0,stockClass=item.stock<=0?'empty':ratio<=.2?'low':'';
    return `<article class="armory-card"><div class="armory-card__image">${armoryImage(item)}</div><div class="armory-card__body"><small>${esc(item.itemType)}</small><h3>${esc(item.label)}</h3><p>${esc(item.description||'Department-issued equipment')}</p><div class="armory-stock ${stockClass}"><span>${item.stock} / ${item.maxStock} in stock</span><span>Issue ${item.issueAmount}</span></div><button data-armory-checkout="${esc(item.itemName)}" ${item.available?'':'disabled'}>${item.available?'Check out equipment':item.stock<item.issueAmount?'Out of stock':'Rank restricted'}</button></div></article>`;
  }).join('')||'<p>No equipment is enabled in this category.</p>';
}
...
window.addEventListener('message',e=>{const d=e.data||{};if(d.action==='legalArmoryOpen'){app.classList.add('hidden');facilityDialogue.classList.add('hidden');facilityPrompt.classList.add('hidden');document.querySelector('#armoryOrg').textContent=d.label||'LEGAL ORGANIZATION';armoryData=d.data||{items:[]};armoryFilter='all';document.querySelectorAll('[data-armory-filter]').forEach(x=>x.classList.toggle('active',x.dataset.armoryFilter==='all'));legalArmory.classList.remove('hidden');armoryManager.classList.add('hidden');renderArmory()}if(d.action==='legalArmoryClose'){legalArmory.classList.add('hidden');armoryManager.classList.add('hidden')}});
