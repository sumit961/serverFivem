...
document.querySelector('#lawMdtVehicleSearch').onclick=async()=>{const r=await post('lawMdtVehicleSearch',{plate:document.querySelector('#lawMdtPlateQuery').value});if(!r?.ok)return notice(r?.error||'Vehicle not found.','error');document.querySelector('#lawMdtIdle').hidden=true;document.querySelector('#lawMdtDashboard').hidden=true;const v=r.vehicle;const bolos=Array.isArray(v.bolos)?v.bolos:(v.bolo?[v.bolo]:[]);mdtWorkspace.innerHTML=`<article class="card law-mdt-vehicle">${bolos.map(b=>`<div class="mdt-record mdt-record--alert"><strong>ACTIVE BOLO · ${esc(b.organizationLabel||b.organizationId||'Law')}</strong><span>${esc(b.reason||b.description||'Active vehicle BOLO')}</span></div>`).join('')}<small>VEHICLE RECORD</small><h2>${esc(v.licenseNumber||v.plate)}</h2><p>${esc(v.label||v.model)}</p><div class="mdt-record"><strong>Owner</strong><span>${esc(v.ownerName)}${v.ownerCid?` · CID ${esc(v.ownerCid)}`:''}</span></div><div class="mdt-record"><strong>Registration</strong><span>${esc(v.licenseNumber||'UNLICENSED')}</span></div><div class="mdt-record"><strong>Impound</strong><span>${v.impound?`${esc(v.impound.organization||v.impound.organizationId||'Law')} · ${esc(v.impound.reason||'')}${v.impound.fee!=null?` · $${Number(v.impound.fee||0).toLocaleString()}`:''}${v.impound.impoundedAt?` · ${esc(v.impound.impoundedAt)}`:''}`:'Not impounded'}</span></div>${v.ownerCid?`<button data-mdt-cid="${esc(v.ownerCid)}">Open owner profile</button>`:''}</article>`};
