...
local function spawnServiceNpc()
    ...
    local model = GetHashKey(PoliceConfig.ServiceNpc.Model or 's_f_y_cop_01')
    RequestModel(model)
    ...
    TaskStartScenarioInPlace(ServicePed, PoliceConfig.ServiceNpc.IdleScenario or 'WORLD_HUMAN_CLIPBOARD', 0, true)
    pcall(SetFacialIdleAnimOverride, ServicePed, 'mood_normal_1', 0)
    SetModelAsNoLongerNeeded(model)
end

local function greeting()
    local settings = PoliceConfig.ServiceNpc.Greetings or {}
    local wanted = 0
    pcall(function() wanted = exports[PoliceConfig.PlayerDataResource]:GetWantedStars() end)
...
CreateThread(function()
    ...
    while true do
        ...
        if ServiceLocation then
            ...
            if distance <= (PoliceConfig.ServiceNpc.DrawDistance or 18.0) then
                wait = 0
                PoliceDrawNpcName(ServiceLocation, PoliceConfig.ServiceNpc.Name or 'Officer Morgan')
                if distance <= (PoliceConfig.ServiceNpc.InteractDistance or 2.5) then
                    if not promptVisible then PoliceShowNpcInteraction('service_npc', PoliceConfig.ServiceNpc.Name, PoliceConfig.ServiceNpc.Role, 'shield-halved'); promptVisible = true end
                    if IsControlJustPressed(0, 38) then
                        PoliceHideNpcInteraction('service_npc'); promptVisible = false
                        PoliceOpenNpcDialogue(ServicePed, { owner = 'service_npc', name = PoliceConfig.ServiceNpc.Name, role = PoliceConfig.ServiceNpc.Role,
                            quote = greeting(),
