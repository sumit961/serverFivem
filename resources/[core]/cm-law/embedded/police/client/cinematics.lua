...
local function loadPreferences()
    ...
    SendNUIMessage({ cmInterface = "police", action = 'cinematicAccessibility:apply', subtitleScale = preferences.subtitleScale })
end
...
RegisterCommand('policecinematics', function()
    ...
    SetNuiFocus(true, true); SendNUIMessage({ cmInterface = "police", action = 'cinematicAccessibility:open', preferences = preferences })
end, false)
RegisterNUICallback('police_closeCinematicAccessibility', function(_, cb) SetNuiFocus(false, false); SendNUIMessage({ cmInterface = "police", action = 'cinematicAccessibility:close' }); cb({ ok = true }) end)
RegisterNUICallback('police_saveCinematicAccessibility', function(data, cb)
    ...
    SendNUIMessage({ cmInterface = "police", action = 'cinematicAccessibility:apply', subtitleScale = preferences.subtitleScale })
    SendNUIMessage({ cmInterface = "police", action = 'cinematicAccessibility:close' }); PoliceNotify('Cinematic accessibility preferences saved.', 'success'); cb({ ok = true })
end)
...
local function finish()
    clearCamera(false)
    SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:hide' })
...
function PolicePreviewCinematic(kind)
    ...
    SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:show', mode = kind, eyebrow = 'ADMIN CAMERA PREVIEW',
    ...
    if active then SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:stamp', label = 'PREVIEW COMPLETE' }); PoliceCinematicSound('confirm'); Wait(1200); finish() end
end
...
function PolicePlayBookingIntakeCinematic(targetServerId, preview)
    ...
    SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:show', mode = 'booking', eyebrow = 'POLICE INTAKE',
    ...
    PoliceCinematicSound('shutter'); if not preferences.reducedFlash then SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:flash' }) end; Wait(450)
    markSeen('booking')
...
RegisterNetEvent('cm-police:client:bookingCompleted', function(summary)
    ...
    SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:show', mode = 'cell', eyebrow = 'CUSTODY CONFIRMED',
    ...
    SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:stamp', label = 'SENTENCE STARTED' })
...
function PolicePlayImpoundArrival(towTruck, vehicle)
    ...
    SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:show', mode = 'impound', eyebrow = 'POLICE IMPOUND', title = 'VEHICLE INTAKE' })
...
function PolicePlayImpoundCompletion(towTruck, summary)
    ...
    SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:update', mode = 'impound', title = 'EVIDENCE FILED',
    ...
    SendNUIMessage({ cmInterface = "police", action = 'policeCinematic:stamp', label = 'IMPOUND COMPLETE' })
