...
local function refresh(override)
    -- cm-prison owns the one shared intake NPC. The embedded Police fallback
    -- is only for servers that do not run cm-prison, preventing duplicate
    -- intake officers when the central resource is installed.
    if GetResourceState('cm-prison') == 'started' then
        location = nil; deleteNpc(); refreshing = false; return
    end
    ...
    local model = GetHashKey((PoliceConfig.JailNpc or {}).Model or 's_m_m_prisguard_01')
    RequestModel(model)
...
CreateThread(function()
    ...
    while true do
        local wait = 1000
        if GetResourceState('cm-prison') == 'started' then
            if npc and DoesEntityExist(npc) then deleteNpc() end
            Wait(1000)
        else
        if npc and DoesEntityExist(npc) then
            local distance = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(npc))
            local settings = PoliceConfig.JailNpc or {}
