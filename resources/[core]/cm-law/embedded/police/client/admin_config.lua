local function apply(npc)
    ...
    if npc.id == 'front_desk' then
        PoliceConfig.ServiceNpc.Model,PoliceConfig.ServiceNpc.Name,PoliceConfig.ServiceNpc.Role=npc.model,npc.name,npc.role
    ...
    elseif npc.id == 'armory' or npc.id == 'storage' then
        ...
        PoliceConfig.FacilityNpcs[prefix..'Model'],PoliceConfig.FacilityNpcs[prefix..'Name'],PoliceConfig.FacilityNpcs[prefix..'Role']=npc.model,npc.name,npc.role
        TriggerEvent('cm-police:client:facilityNpcUpdated',npc.id..'_npc',location)
    elseif npc.id == 'prison_intake' then
        PoliceConfig.JailNpc.Model,PoliceConfig.JailNpc.Name,PoliceConfig.JailNpc.Role=npc.model,npc.name,npc.role
        TriggerEvent('cm-police:client:jailNpcUpdated',location)
    elseif npc.id == 'wardrobe' then
        PoliceConfig.Wardrobe.NpcModel,PoliceConfig.Wardrobe.NpcName,PoliceConfig.Wardrobe.NpcRole=npc.model,npc.name,npc.role
        TriggerEvent('cm-police:client:wardrobeNpcUpdated',location)
    elseif npc.id == 'impound' then
        PoliceConfig.Impound.OperatorModel,PoliceConfig.Impound.OperatorName,PoliceConfig.Impound.OperatorRole=npc.model,npc.name,npc.role
