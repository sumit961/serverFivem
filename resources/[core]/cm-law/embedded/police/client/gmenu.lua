...
RegisterNetEvent('cm-police:client:bookingIntake', function(targetServerId)
    -- Police uses the same shared charge review and central prison intake as
    -- every other legal organization. Keep the legacy event as a compatibility
    -- alias so old interaction registrations cannot open a second booking UI.
    TriggerEvent('cm-law:client:bookingIntake', targetServerId)
end)

function rebuild(targetServerId)
    if GetResourceState(PoliceConfig.PlayerDataResource) ~= 'started' then return end
    exports[PoliceConfig.PlayerDataResource]:ClearInteractionOptions(PAGE)
    targetServerId = tonumber(targetServerId)
...
AddEventHandler('onClientResourceStart', function(resource)
    if resource == GetCurrentResourceName() or resource == PoliceConfig.PlayerDataResource then Wait(300); registerPage(); rebuild(nil) end
end)
