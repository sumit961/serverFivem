...
CreateThread(function()
    while true do
        ...
        if IsPedShooting(ped) then
            ...
            local windowMs = PoliceConfig.Dispatch.GunfireWindowMs or 10000
            while shotTimestamps[1] and now - shotTimestamps[1] > windowMs do
                table.remove(shotTimestamps, 1)
            end
            local threshold = PoliceConfig.Dispatch.GunfireShotThreshold or 8
            local cooldown = PoliceConfig.Dispatch.GunfireCooldownMs or 120000
