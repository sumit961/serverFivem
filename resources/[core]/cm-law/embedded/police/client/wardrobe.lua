...
local function openCamera()
    ...
    local dist = PoliceConfig.Wardrobe.CameraDistance or 4.35
    local heightOffset = PoliceConfig.Wardrobe.CameraHeightOffset or 0.18
    ...
    cam = CreateCamWithParams('DEFAULT_SCRIPTED_CAMERA', camPos.x, camPos.y, camPos.z, 0.0, 0.0, 0.0, PoliceConfig.Wardrobe.CameraFov or 38.0, false, 0)
...
RegisterNUICallback('police_openWardrobeDressingRoom', function(_, cb)
...
RegisterNUICallback('police_openPoliceNpcCloset', function(_, cb)
    if roomOpen then return cb({ ok = false }) end
...
RegisterNUICallback('police_saveWardrobeFavorite', function(data, cb)
...
RegisterNUICallback('police_removeWardrobeFavorite', function(data, cb)
    local ok, message, favorites = lib.callback.await('cm-police:server:removeWardrobeFavorite', false, data and data.slot)
    notify(message, ok and 'success' or 'error')
    cb({ ok = ok == true, favorites = favorites or {} })
end)

RegisterNUICallback('police_wearWardrobeFavorite', function(data, cb)
...
RegisterNUICallback('police_finishWardrobeDuty', function(_, cb)
...
RegisterNUICallback('police_closeWardrobeDressingRoom', function(_, cb)
...
RegisterNUICallback('police_rotateWardrobePed', function(data, cb)
...
local function spawnWardrobeNpc()
    ...
    local hash = GetHashKey(PoliceConfig.Wardrobe.NpcModel or 'mp_m_shopkeep_01')
    RequestModel(hash)
...
local function openManageSlots(data)
    local options = {}
    for slot = 1, (PoliceConfig.Wardrobe.MaxQuickSlots or 5) do
...
local function OpenPoliceVisualCloset()
    SendNUIMessage({ cmInterface = "police", action = 'policeCloset:open' })
...
CreateThread(function()
    while true do
        ...
        if NpcLocation and not roomOpen then
            ...
            if dist <= 12.0 then
                wait = 0
                PoliceDrawNpcName(NpcLocation, PoliceConfig.Wardrobe.NpcName or 'Officer Taylor')
                if dist <= (PoliceConfig.Wardrobe.NpcInteractDistance or 2.5) then
                    if not wardrobePromptVisible then
                        PoliceShowNpcInteraction('wardrobe_npc', PoliceConfig.Wardrobe.NpcName, PoliceConfig.Wardrobe.NpcRole, 'shirt')
                        wardrobePromptVisible = true
                    end
                    if IsControlJustPressed(0, 38) then
                        PoliceHideNpcInteraction('wardrobe_npc'); wardrobePromptVisible = false
                        PoliceOpenRestrictedNpcDialogue(npcPed, { owner = 'wardrobe_npc', name = PoliceConfig.Wardrobe.NpcName, role = PoliceConfig.Wardrobe.NpcRole,
