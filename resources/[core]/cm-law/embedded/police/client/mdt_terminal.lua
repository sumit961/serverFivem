...
local function closeTerminal()
    ...
    open = false
    SendNUIMessage({ cmInterface = "police", action = 'closeMdtTerminal' })
    restoreFocus()
end
...
local function togglePoliceMdt()
    ...
    SendNUIMessage({ cmInterface = "police", action = 'openMdtTerminal', data = data })
end

RegisterCommand('policemdtterminal', togglePoliceMdt, false)
RegisterNetEvent('cm-police:client:toggleMdt', togglePoliceMdt)

RegisterNUICallback('police_closeMdtTerminal', function(_, cb)
    closeTerminal()
    cb({ ok = true })
end)
