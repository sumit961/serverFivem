...
local function npcPresentation(kind)
    if kind == 'armory_npc' then
        return PoliceConfig.FacilityNpcs.ArmoryName or 'Officer Hayes', PoliceConfig.FacilityNpcs.ArmoryRole or 'Police Quartermaster', 'gun'
    end
    return PoliceConfig.FacilityNpcs.StorageName or 'Officer Brooks', PoliceConfig.FacilityNpcs.StorageRole or 'Police Storekeeper', 'box-archive'
end
...
local function spawnPed(kind)
    ...
    local modelName = kind == 'armory_npc' and PoliceConfig.FacilityNpcs.ArmoryModel or PoliceConfig.FacilityNpcs.StorageModel
    ...
    while true do
        ...
        for kind, location in pairs(Locations) do
            if location then
                ...
                if distance <= (PoliceConfig.FacilityNpcs.DrawDistance or 18.0) then
                    wait = 0
                    local name, role, icon = npcPresentation(kind)
                    PoliceDrawNpcName(location, name)
                    if distance <= (PoliceConfig.FacilityNpcs.InteractDistance or 2.5) then
        ...
        if PromptKind then
            ...
            if not active or #(coords - vector3(active.x, active.y, active.z)) > (PoliceConfig.FacilityNpcs.InteractDistance or 2.5) then
