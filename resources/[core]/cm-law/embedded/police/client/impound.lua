...
local function configuredTowTruck(vehicle)
    ...
    for _, name in ipairs(PoliceConfig.Impound.TowModels or {}) do
...
local function closestTowTarget(towTruck)
    local origin, best, bestDistance = GetEntityCoords(towTruck), nil, PoliceConfig.Impound.TowAttachDistance or 10.0
...
local function openImpoundMenu()
    ...
    SendNUIMessage({ cmInterface = "police", action = 'impoundRelease:open', vehicles = vehicles })
end
...
local function closeImpoundRelease()
    ...
    SendNUIMessage({ cmInterface = "police", action = 'impoundRelease:close' })
end

RegisterNUICallback('police_closeImpoundRelease', function(_, cb)
    closeImpoundRelease()
    cb({ ok = true })
end)

RegisterNUICallback('police_payImpoundRelease', function(data, cb)
    ...
    if ok then
        ...
        else SendNUIMessage({ cmInterface = "police", action = 'impoundRelease:open', vehicles = vehicles }) end
    end
    cb({ ok = ok == true })
end)
...
local function refreshOperators()
    ...
    local model = GetHashKey(PoliceConfig.Impound.OperatorModel or 's_m_y_cop_01')
    RequestModel(model)
    ...
    while true do
        ...
        if not TowTarget and #KioskLocations > 0 then
            ...
            if towTruck ~= 0 and GetPedInVehicleSeat(towTruck, -1) == ped and configuredTowTruck(towTruck)
                and nearOperator(towTruck, PoliceConfig.Impound.DropoffRadius or 18.0) then
    ...
    while true do
        ...
        if not impoundReleaseOpen and #KioskLocations > 0 and not IsPedInAnyVehicle(PlayerPedId(), false) then
            ...
            if nearest and nearestDistance <= (PoliceConfig.Impound.OperatorDrawDistance or 7.0) then
                ...
                PoliceDrawNpcName(location, PoliceConfig.Impound.OperatorName or 'Officer Martinez')
                if nearestDistance <= (PoliceConfig.Impound.KioskRadius or 2.5) then
                    if promptIndex ~= nearest then
                        if promptIndex then PoliceHideNpcInteraction('impound_operator') end
                        PoliceShowNpcInteraction('impound_operator', PoliceConfig.Impound.OperatorName or 'Officer Martinez', PoliceConfig.Impound.OperatorRole or 'Impound Operator', 'car')
                        promptIndex = nearest
                    end
                    if IsControlJustPressed(0, PoliceConfig.Impound.KioskInteractKey or 38) and ped and DoesEntityExist(ped) then
                        PoliceHideNpcInteraction('impound_operator'); promptIndex = nil
                        PoliceOpenNpcDialogue(ped, { owner = 'impound_operator', name = PoliceConfig.Impound.OperatorName,
                            role = PoliceConfig.Impound.OperatorRole, quote = 'I can help you review and release vehicles currently held in Police impound.',
