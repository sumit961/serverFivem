...
PoliceConfig.PlayerDataResource = 'cm-playerdata'
PoliceConfig.AdminResource = 'cm-admin'
PoliceConfig.AdminPermission = 'police.admin.manage'
PoliceConfig.VehiclesResource = 'cm-vehicles'
PoliceConfig.ShopResource = 'rn-vehicleshop'
...
-- see server/main.lua.
PoliceConfig.OrganizationId = 'police'
PoliceConfig.MenuCommand = 'police'
PoliceConfig.MenuKey = 'F6'
...
PoliceConfig.CuffKey = 'X'
PoliceConfig.LogLimit = 100
...
PoliceConfig.EvidenceRetentionDays = 30   -- delete cm_police_evidence rows + their .jpg after this many days
PoliceConfig.LogRetentionDays = 90        -- delete cm_police_activity rows after this many days
PoliceConfig.IncidentRetentionDays = 30   -- delete closed/expired dispatch calls after this many days
PoliceConfig.RetentionSweepMs = 21600000  -- how often to sweep (6 hours)

PoliceConfig.Permissions = {
...
-- Tickets / citations (PoliceConfig.Permissions.police.cite gates who can issue
...
-- Vehicle impound & tow (PoliceConfig.Permissions.police.impound gates who can
...
PoliceConfig.Impound = {
    ...
    -- Public release kiosk (PoliceConfig.Permissions.police.manage_impound gates who
...
-- Speed radar (PoliceConfig.Permissions.police.radar gates who can use it).
...
-- Spike strips (PoliceConfig.Permissions.police.spike gates who can use it). No
...
-- Barricades (PoliceConfig.Permissions.police.barricade gates who can deploy
...
PoliceConfig.Barricades = {
    ...
    -- PoliceConfig.Ranks) -- admins add/remove further models from the F7
    -- dashboard's Barricades card afterwards.
    DefaultModels = { 'prop_barrier_work05' },
}

-- Wheel clamp (PoliceConfig.Permissions.police.clamp gates who can toggle it).
...
PoliceConfig.Placement = {
    MinDistance = 1.0,
    MaxDistance = 5.0,
}

-- MDT records lookup (PoliceConfig.Permissions.police.mdt gates who can use it --
...
-- 911/dispatch (PoliceConfig.Permissions.police.receive_dispatch gates who
...
PoliceConfig.Backup = {
    ...
    BlipColour = 1, -- red -- visually distinct from PoliceConfig.Dispatch.BlipColour (3)
}

-- ALPR (automatic license plate reader) cameras (PoliceConfig.Permissions.police.manage_alpr
...
-- K9 unit (PoliceConfig.Permissions.police.k9 gates who can deploy/command one,
...
PoliceConfig.Fto = {
    RestrictedTier = 1, -- matches PoliceConfig.Ranks' Cadet tier
}
