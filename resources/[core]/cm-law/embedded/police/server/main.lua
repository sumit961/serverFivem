...
function cid(src)
    local ok, value = pcall(function()
        return exports[PoliceConfig.PlayerDataResource]:GetCharacterId(tonumber(src))
    end)
...
-- cross-file-global convention as cid/PoliceLegacyMemberFor/has/log/PoliceLegacyRateLimit/PoliceLegacyNameFor.
function PoliceLegacySourceFor(characterId)
    local ok, value = pcall(function()
        return exports[PoliceConfig.PlayerDataResource]:GetSourceByCharId(characterId)
    end)
    return ok and tonumber(value) or nil
end

function PoliceLegacyNameFor(characterId)
    local src = PoliceLegacySourceFor(characterId)
    if src then
        local ok, value = pcall(function()
            return exports[PoliceConfig.PlayerDataResource]:GetCharacterFullName(src)
        end)
...
function PoliceLegacyDbBoolean(value)
...
function PoliceBeginDutyWithOutfit(characterId, outfit, detail)
    ...
    if PoliceLegacyDbBoolean(member.is_suspended) then
    ...
    if not PoliceLegacyDbBoolean(member.on_duty) then
    ...
    return true, PoliceLegacyDbBoolean(member.on_duty) and 'Police clothing applied.' or 'Police clothing applied. You are now on duty.', clean
end

local function permissionMap(rank)
    if not rank then return {} end
    if PoliceLegacyDbBoolean(rank.is_leader) then
        local all = {}
        for key in pairs(PoliceConfig.Permissions) do all[key] = true end
        return all
    end
...
function PoliceLegacyMemberFor(characterId)
...
-- PoliceLegacyMemberFor(), which now includes fto_signed_off.
function isFtoRestricted(member)
    return member ~= nil and tonumber(member.tier) == (PoliceConfig.Fto.RestrictedTier or 1) and not PoliceLegacyDbBoolean(member.fto_signed_off)
end
...
local function stateFor(characterId)
    ...
    return {
        ...
        isLeader = PoliceLegacyDbBoolean(member.is_leader),
        onDuty = PoliceLegacyDbBoolean(member.on_duty),
        suspended = PoliceLegacyDbBoolean(member.is_suspended),
...
sync = function(characterId)
    ...
    -- own name-privacy system (PoliceConfig.Interactions.PrivacyMode, config.lua)
    ...
    pcall(function()
        if state then
            exports[PoliceConfig.PlayerDataResource]:SetOrganization(src, PoliceConfig.OrganizationId, 'Police Department')
        else
            exports[PoliceConfig.PlayerDataResource]:SetOrganization(src, nil, nil)
        end
    end)
end
...
function PoliceLegacyRateLimit(src, action, waitMs)
...
local function setupDatabase()
    ...
    if existingRankCount == 0 then
        for _, rank in ipairs(PoliceConfig.Ranks) do
    ...
    if not MySQL.scalar.await('SELECT migration_key FROM cm_police_migrations WHERE migration_key = ? LIMIT 1', { 'v1.1_cuff_permission' }) then
        ...
        for _, rank in ipairs(rows) do
            if not PoliceLegacyDbBoolean(rank.is_leader) then
                ...
                for key, value in pairs(stored) do
                    ...
                    if enabled and PoliceConfig.Permissions[permission] and not seen[permission] then seen[permission] = true; permissions[#permissions + 1] = permission end
                end
    ...
    -- PoliceConfig.Ranks earlier this session, but editing that file alone never
    -- reaches an already-running server's existing rank rows (only a
    -- completely empty cm_police_ranks table gets seeded from PoliceConfig.Ranks
    ...
    if not MySQL.scalar.await('SELECT migration_key FROM cm_police_migrations WHERE migration_key = ? LIMIT 1', { 'v1.8_armory_permission' }) then
        ...
        -- rank, matching PoliceConfig.Ranks' own intent for those permissions),
        ...
        for _, rank in ipairs(rows) do
            if not PoliceLegacyDbBoolean(rank.is_leader) and tonumber(rank.tier) and tonumber(rank.tier) >= 80 then
                local stored, seen, permissions = decode(rank.permissions), {}, {}
                for key, value in pairs(stored) do
                    local permission = type(key) == 'number' and tostring(value) or tostring(key)
                    local enabled = type(key) == 'number' or value == true
...
local function rankRows()
    ...
    for _, rank in ipairs(rows) do
        ...
        rank.isLeader = PoliceLegacyDbBoolean(rank.is_leader)
...
local function roster()
    ...
    for _, row in ipairs(rows) do
        ...
        row.name = PoliceLegacyNameFor(row.character_id)
        row.rankName = tostring(row.rank_name or 'Police Member')
        row.rankId, row.tier = tonumber(row.rank_id), tonumber(row.tier)
        row.onDuty = PoliceLegacyDbBoolean(row.on_duty)
        row.isLeader = PoliceLegacyDbBoolean(row.is_leader)
        row.suspended = PoliceLegacyDbBoolean(row.is_suspended)
        row.suspendedUntil = row.suspended_until and tostring(row.suspended_until) or nil
        row.suspensionReason = row.suspension_reason
        row.online = PoliceLegacySourceFor(row.character_id) ~= nil
        row.radioStatus = radioStatusFor(row.character_id)
        row.ftoSignedOff = PoliceLegacyDbBoolean(row.fto_signed_off)
...
local function dashboard(src, adminMode, requestedSex)
    ...
    local memberSuspended = member and PoliceLegacyDbBoolean(member.is_suspended) or false
    ...
    local logs = {}
    if canViewLogs then
        local limit = math.max(1, math.min(tonumber(PoliceConfig.LogLimit) or 100, 250))
    ...
    if isAdmin and not dashboardSelf then
        ...
        for permission in pairs(PoliceConfig.Permissions) do adminPermissions[permission] = true end
        dashboardSelf = {
    ...
    return {
        organization = { name = 'Police Department', leaderCid = org.leader_cid and tostring(org.leader_cid) or nil, leaderName = org.leader_cid and PoliceLegacyNameFor(org.leader_cid) or 'Not assigned' },
        ...
        permissions = PoliceConfig.Permissions, adminMode = isAdmin,
        ...
        violations = (function()
            local list = {}
            for _, violation in ipairs(PoliceConfig.Citations.Violations) do
        ...
        useOfForceTypes = PoliceConfig.Mdt.UseOfForceTypes,
        capabilities = {
...
local function targetChange(actorCid, targetCid, direction)
    local actor, target = PoliceLegacyMemberFor(actorCid), PoliceLegacyMemberFor(targetCid)
    ...
    if PoliceLegacyDbBoolean(target.is_leader) then return false, 'The Police leader rank is admin-managed.' end
    ...
    return true, ('%s is now %s.'):format(PoliceLegacyNameFor(targetCid), rank.name)
end
...
local function requestedPermissions(raw, actor)
    ...
    if type(raw) == 'table' then
        for key, value in pairs(raw) do
            local permission = type(key) == 'number' and tostring(value) or tostring(key)
            local enabled = type(key) == 'number' or value == true
            if enabled and PoliceConfig.Permissions[permission] then requested[permission] = true end
        end
    end
...
lib.callback.register('cm-police:server:diagnostics', function(src)
    local allowed = false
    pcall(function() allowed = exports[PoliceConfig.AdminResource]:HasPermission(src, PoliceConfig.AdminPermission) == true end)
...
policeK9Member = function(src)
    ...
    local characterId = cid(src)
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or PoliceLegacyDbBoolean(member.is_suspended) or not PoliceLegacyDbBoolean(member.on_duty) or not has(member, 'police.k9') then
...
lib.callback.register('cm-police:server:deployK9', function(src)
    ...
    local ok, message, netId = xpcall(function()
        ...
        local dog = CreatePed(28, GetHashKey(PoliceConfig.K9.Model or 'a_c_shepherd'), spawnX, spawnY, coords.z, heading, true, true)
...
lib.callback.register('cm-police:server:officerStats', function(src)
    ...
    for _, row in ipairs(rows) do
        ...
        list[#list + 1] = {
            characterId = tostring(row.actor_cid), name = PoliceLegacyNameFor(row.actor_cid),
...
lib.callback.register('cm-police:server:k9Track', function(src)
    if not PoliceLegacyRateLimit(src, 'k9_track', 1000) then return false, 'Please wait.' end
    local record, err = livePoliceK9(src)
    ...
    local nearestSrc, nearestCoords, nearestDist = nil, nil, PoliceConfig.K9.TrackRadius or 150.0
...
lib.callback.register('cm-police:server:k9TrackUpdate', function(src, targetSrc)
    if not PoliceLegacyRateLimit(src, 'k9_track_update', 750) then return false end
    local _, targetPed, err = validateK9PlayerTarget(src, targetSrc, PoliceConfig.K9.TrackRadius or 150.0)
...
lib.callback.register('cm-police:server:k9CommandTarget', function(src, action, targetSrc)
    if not PoliceLegacyRateLimit(src, 'k9_command', 500) then return false, 'Please wait.' end
    action = tostring(action or '')
    if action ~= 'follow' and action ~= 'chase' and action ~= 'attack' then return false, 'Invalid K9 command.' end
    local maxDistance = action == 'attack' and (PoliceConfig.K9.AttackDistance or 20.0) or (PoliceConfig.K9.ChaseDistance or 75.0)
...
lib.callback.register('cm-police:server:k9SearchPlayer', function(src, targetSrc)
    if not PoliceLegacyRateLimit(src, 'k9_search_player', 1500) then return false, 'Please wait.' end
    local _, _, err = validateK9PlayerTarget(src, targetSrc, PoliceConfig.K9.SearchDistance or 4.0)
    if err then return false, err end
    targetSrc = tonumber(targetSrc)
...
lib.callback.register('cm-police:server:k9SearchVehicle', function(src, netId)
    if not PoliceLegacyRateLimit(src, 'k9_search_vehicle', 1500) then return false, 'Please wait.' end
    local record, err = livePoliceK9(src)
    if not record then return false, err end
    ...
    local handlerPed = GetPlayerPed(src)
    if #(GetEntityCoords(handlerPed) - GetEntityCoords(vehicle)) > (PoliceConfig.K9.SearchDistance or 4.0) then return false, 'Vehicle is too far away.' end
...
AddEventHandler('weaponDamageEvent', function(sender, data)
    ...
    for handlerSrc in pairs(PoliceK9BySource) do
        if sender ~= handlerSrc and victim == GetPlayerPed(handlerSrc)
            ...
            PoliceK9Threats[handlerSrc][sender] = GetGameTimer() + (PoliceConfig.K9.ThreatMemoryMs or 120000)
        end
    end
end)
...
CreateThread(function()
    while true do
        ...
        for handlerSrc, targetSrc in pairs(PoliceK9AttackTargets) do
            local _, _, err = validateK9PlayerTarget(handlerSrc, targetSrc, PoliceConfig.K9.AttackDistance or 20.0)
...
lib.callback.register('cm-police:server:setRadioStatus', function(src, status)
    ...
    if not actor or PoliceLegacyDbBoolean(actor.is_suspended) or not PoliceLegacyDbBoolean(actor.on_duty) then
    ...
    sync(actorCid)
    TriggerEvent('cm-law:server:legacyUnitStatusChanged', src, actorCid, status)
    return true, ('Status set to %s.'):format(status)
end)

AddEventHandler('cm-police:server:liveUnitStatusChanged', function(characterId, status)
    characterId = tostring(characterId or '')
    if characterId == '' then return end
    OfficerRadioStatus[characterId] = (status == 'busy' or status == 'unavailable' or status == 'en_route' or status == 'on_scene') and '10-6' or '10-8'
    sync(characterId)
end)
...
local function endPoliceDuty(src, characterId, reason)
    ...
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or not PoliceLegacyDbBoolean(member.on_duty) then return false end
...
RegisterNetEvent('cm-police:server:endDutyOnDeath', function()
    local src = source
    if not PoliceLegacyRateLimit(src, 'death_end_duty', 2000) then return end
    local characterId = cid(src)
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or not PoliceLegacyDbBoolean(member.on_duty) then return end
...
lib.callback.register('cm-police:server:dutyOutfit', function(src, requestedSex)
    local characterId = cid(src)
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or not PoliceLegacyDbBoolean(member.on_duty) then return nil end
    local outfitSex = requestedSex == 'female' and 'female' or 'male'
...
lib.callback.register('cm-police:server:policeWardrobeCatalog', function(src, requestedSex)
    ...
    local isAdmin = false
    pcall(function() isAdmin = exports[PoliceConfig.AdminResource]:HasPermission(src, PoliceConfig.AdminPermission) == true end)
    if not isAdmin and (not member or PoliceLegacyDbBoolean(member.is_suspended)) then return {} end
...
lib.callback.register('cm-police:server:action', function(src, action, payload)
    if not PoliceLegacyRateLimit(src, tostring(action), 650) then return false, 'Please wait.' end
    ...
    elseif action == 'set_meeting' then
        ...
        local recipients = 0
        local setterName = PoliceLegacyNameFor(actorCid)
        for _, player in ipairs(GetPlayers()) do
            local memberCid = cid(player)
            if memberCid and PoliceLegacyMemberFor(memberCid) then
    ...
    elseif action == 'save_rank' then
        ...
        if rankId then
            ...
            if PoliceLegacyDbBoolean(existing.is_leader) then return false, 'The Police leader rank is protected.' end
    ...
    elseif action == 'delete_rank' then
        ...
        if PoliceLegacyDbBoolean(rank.is_leader) then return false, 'The Police leader rank cannot be deleted.' end
    ...
    elseif action == 'kick' then
        ...
        if not target or PoliceLegacyDbBoolean(target.is_leader) or tonumber(actor.tier) <= tonumber(target.tier) then return false, 'You can only remove lower-ranked members.' end
        endPoliceDuty(PoliceLegacySourceFor(targetCid), targetCid, 'removed')
        ...
        return true, ('%s was removed from Police.'):format(PoliceLegacyNameFor(targetCid))
    ...
    elseif action == 'suspend_member' then
        ...
        local targetCid = tostring(payload.characterId or '')
        local target = PoliceLegacyMemberFor(targetCid)
        if not target or PoliceLegacyDbBoolean(target.is_leader) or tonumber(actor.tier) <= tonumber(target.tier) then return false, 'You can only suspend lower-ranked members.' end
        ...
        endPoliceDuty(PoliceLegacySourceFor(targetCid), targetCid, 'suspended')
        ...
        return true, ('%s was suspended for %d minutes.'):format(PoliceLegacyNameFor(targetCid), minutes)
    elseif action == 'reinstate_member' then
        ...
        local targetCid = tostring(payload.characterId or '')
        local target = PoliceLegacyMemberFor(targetCid)
        if not target or PoliceLegacyDbBoolean(target.is_leader) or tonumber(actor.tier) <= tonumber(target.tier) then return false, 'You can only reinstate lower-ranked members.' end
        ...
        return true, ('%s was reinstated.'):format(PoliceLegacyNameFor(targetCid))
    elseif action == 'sign_off_cadet' then
        ...
        return true, ('%s is signed off.'):format(PoliceLegacyNameFor(targetCid))
    end
...
AddEventHandler('cm-police:server:gMenuAction', function(src, targetSrc, action, _, context)
    ...
    elseif action == 'police_kick' then
        if not has(actor, 'police.kick') then ok, message = false, 'Your rank cannot remove members.'
            ...
            local target = PoliceLegacyMemberFor(targetCid)
            if not target or PoliceLegacyDbBoolean(target.is_leader) or tonumber(actor.tier) <= tonumber(target.tier) then ok, message = false, 'You can only remove lower-ranked members.'
            else MySQL.update.await('DELETE FROM cm_police_members WHERE character_id = ?', { targetCid }); sync(targetCid); log(actorCid, 'member_removed', { targetCid = targetCid }); ok, message = true, ('%s was removed from Police.'):format(PoliceLegacyNameFor(targetCid)) end
        end
    end
...
lib.callback.register('cm-police:server:respondInvite', function(src, accept)
    ...
    local inviterSrc = PoliceLegacySourceFor(invite.invited_by)
    local inviter = inviterSrc and PoliceLegacyMemberFor(invite.invited_by) or nil
    if not inviterSrc or not inviter or PoliceLegacyDbBoolean(inviter.is_suspended) or not has(inviter, 'police.invite') then
...
RegisterCommand(PoliceConfig.AdminMenuCommand or 'policeadmin', function(src)
    src = tonumber(src)
...
exports('PoliceLegacyGetMember', function(characterId) return stateFor(tostring(characterId)) end)
exports('PoliceLegacyHasPermission', function(characterId, permission) local member = PoliceLegacyMemberFor(tostring(characterId)); return has(member, tostring(permission)) end)
exports('PoliceLegacyIsOnDuty', function(characterId) local member = PoliceLegacyMemberFor(tostring(characterId)); return member and PoliceLegacyDbBoolean(member.on_duty) or false end)
...
exports('CanUseVehicle', function(src, vehicleId, action)
    ...
    local characterId = cid(src)
    local member = characterId and PoliceLegacyMemberFor(characterId)
    return member ~= nil and not PoliceLegacyDbBoolean(member.is_suspended) and PoliceLegacyDbBoolean(member.on_duty) and has(member, 'police.mdt')
end)
-- For cm-admin's centralized Organizations tab (CMOrganizations.forAdminPayload).
exports('PoliceLegacyGetOrganizationSummary', function()
    ...
    return {
        ...
        leaderName = org.leader_cid and PoliceLegacyNameFor(org.leader_cid) or nil,
...
lib.callback.register('cm-police:server:adminStaffAction', function(src, action, payload)
    if not isPoliceAdmin(src) or not PoliceLegacyRateLimit(src, 'police_admin_staff', 800) then return false, 'Permission denied.' end
    payload = type(payload) == 'table' and payload or {}
    action = tostring(action or '')
    local targetCid = tostring(payload.characterId or '')
    ...
    if action == 'hire' then
        ...
        return true, ('%s was hired as %s.'):format(PoliceLegacyNameFor(targetCid), rank.name)
    end
    if not target then return false, 'That character is not a Police member.' end
    if PoliceLegacyDbBoolean(target.is_leader) then return false, 'Assign a new leader before changing the current leader.' end
    if action == 'fire' then
        MySQL.update.await('DELETE FROM cm_police_members WHERE character_id = ?', { targetCid })
