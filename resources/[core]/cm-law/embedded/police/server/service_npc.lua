...
local function nearServiceNpc(src)
    ...
    return #(GetEntityCoords(ped) - vector3(location.x, location.y, location.z)) <= (PoliceConfig.ServiceNpc.InteractDistance or 2.5) + 1.0
end
...
lib.callback.register('cm-police:server:confiscationPreview', function(src)
    if not nearServiceNpc(src) or not PoliceLegacyRateLimit(src, 'police_confiscation_preview', 700) then return nil, 'You must be at the Police front desk.' end
...
lib.callback.register('cm-police:server:confiscateIllegalWeapons', function(src, token)
    ...
    if not nearServiceNpc(src) or not PoliceLegacyRateLimit(src, 'police_confiscation_execute', 1000) then return false, 'You must be at the Police front desk.' end
    local weapons, licensed, failure = firearmInventory(src)
...
lib.callback.register('cm-police:server:surrenderPreview', function(src)
    if not nearServiceNpc(src) or not PoliceLegacyRateLimit(src, 'police_surrender_preview', 700) then return nil, 'You must be at the Police front desk.' end
    local characterId = cid(src)
    ...
    if stars < 1 or not PoliceLegacyDbBoolean(status and status.wanted) then return nil, 'You do not have an active wanted level.' end
...
lib.callback.register('cm-police:server:surrenderAtNpc', function(src)
    if not nearServiceNpc(src) or not PoliceLegacyRateLimit(src, 'police_surrender_execute', 1500) then return false, 'You must be at the Police front desk.' end
    local characterId = cid(src)
    local status = characterId and MySQL.single.await('SELECT stars, wanted FROM cm_police_criminal_status WHERE character_id = ? LIMIT 1', { characterId })
    local stars = status and math.max(0, math.min(6, math.floor(tonumber(status.stars) or 0))) or 0
    if stars < 1 or not PoliceLegacyDbBoolean(status and status.wanted) then return false, 'You do not have an active wanted level.' end
    ...
    pcall(function() exports[PoliceConfig.PlayerDataResource]:ClearWantedStars(src) end)
