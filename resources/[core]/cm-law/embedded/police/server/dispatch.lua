...
-- /reportcrime instead, see shared/config.lua's PoliceConfig.Dispatch comment),
...
-- cid/PoliceLegacyMemberFor/has/log/PoliceLegacyRateLimit/PoliceLegacyNameFor from server/main.lua.
function recipients(permission)
    ...
    for _, rawSrc in ipairs(GetPlayers()) do
        ...
        if targetSrc then
            ...
            if member and not PoliceLegacyDbBoolean(member.is_suspended) and PoliceLegacyDbBoolean(member.on_duty) and has(member, permission) then
                out[#out + 1] = targetSrc
            end
        end
    end
...
exports('CreatePoliceCall', function(src, details)
    ...
    if not PoliceLegacyRateLimit(src, 'police_shared_911', PoliceConfig.Dispatch.Cooldown or 30000) then return false, 'You already contacted emergency services recently.' end
    ...
    local call, reason = createCall(cleanDetails(details), GetEntityCoords(ped), callerCid,
        callerCid and PoliceLegacyNameFor(callerCid) or GetPlayerName(src) or 'Unknown caller')
...
RegisterCommand('reportcrime', function(src, args)
    ...
    if not PoliceLegacyRateLimit(src, 'police_report_crime', PoliceConfig.Dispatch.Cooldown or 30000) then
    ...
    local callerCid = cid(src)
    local callerName = callerCid and PoliceLegacyNameFor(callerCid) or GetPlayerName(src) or 'Unknown caller'
...
-- player's own shots cross PoliceConfig.Dispatch.GunfireShotThreshold within
...
RegisterNetEvent('cm-police:server:reportGunfire', function(x, y, z)
    local src = source
    if not PoliceLegacyRateLimit(src, 'police_gunfire_report', PoliceConfig.Dispatch.GunfireCooldownMs or 120000) then return end
    ...
    if #(serverCoords - reportedCoords) > (PoliceConfig.Dispatch.GunfireCoordinateTolerance or 12.0) then
...
lib.callback.register('cm-police:server:acceptDispatchCall', function(src, callId)
    ...
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or PoliceLegacyDbBoolean(member.is_suspended) or not PoliceLegacyDbBoolean(member.on_duty) or not has(member, 'police.receive_dispatch') then
        return false, 'You must be an on-duty officer with dispatch access.'
    end
    if not call.responders[characterId] then
        call.responders[characterId] = { name = PoliceLegacyNameFor(characterId), source = src, acceptedAt = os.time(), status = 'accepted' }
    end
...
lib.callback.register('cm-police:server:dispatchHistory', function(src)
    ...
    local limit = math.max(1, math.min(tonumber(PoliceConfig.Dispatch.HistoryLimit) or 50, 200))
...
-- expiring after PoliceConfig.Backup.BlipLifetimeMs with no further server input.
lib.callback.register('cm-police:server:requestBackup', function(src, requestedPriority)
    local characterId = cid(src)
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or PoliceLegacyDbBoolean(member.is_suspended) or not PoliceLegacyDbBoolean(member.on_duty) then
    ...
    local cooldown = priority == 'panic' and (PoliceConfig.Backup.PanicCooldown or 30000) or (PoliceConfig.Backup.Cooldown or 60000)
    if not PoliceLegacyRateLimit(src, 'police_request_backup_' .. priority, cooldown) then
    ...
    local coords = GetEntityCoords(ped)
    local name = PoliceLegacyNameFor(characterId)
...
CreateThread(function()
    while true do
        ...
        for callId, call in pairs(ActiveCalls) do
            if now - call.createdAt >= math.floor((PoliceConfig.Dispatch.ExpireAfterMs or 600000) / 1000) then
                call.status = 'expired'
                persistIncident(call)
                ActiveCalls[callId] = nil
                broadcastToRecipients('cm-police:client:dispatchCallResolved', callId)
