...
-- established precedent this resource's own PoliceLegacyNameFor() already uses (no
...
local function authorizedForMdt(src)
    ...
    if member and not PoliceLegacyDbBoolean(member.is_suspended) and PoliceLegacyDbBoolean(member.on_duty) and has(member, 'police.mdt') then
...
function PurchaseLicense(characterId, licenseType, src)
    ...
    local price = tonumber(PoliceConfig.Mdt.LicensePrices and PoliceConfig.Mdt.LicensePrices[licenseType])
    ...
    local removed = false
    pcall(function() removed = exports[PoliceConfig.PlayerDataResource]:RemoveMoney(src, 'bank', price, 'license_purchase', { licenseType = licenseType }) end)
    if not removed then
    ...
    if not licenseNumber then
        ...
        if not licenseNumber then
            ...
            pcall(function() refunded = exports[PoliceConfig.PlayerDataResource]:AddMoney(src, 'bank', price, 'license_purchase_refund', {}) == true end)
...
lib.callback.register('cm-police:server:mdtSearch', function(src, query)
    local authorized = authorizedForMdt(src)
    if not authorized or not PoliceLegacyRateLimit(src, 'police_mdt_search', 600) then return {} end
    query = tostring(query or ''):gsub('^%s+', ''):gsub('%s+$', '')
    if query == '' then return {} end
    local limit = math.max(1, math.min(tonumber(PoliceConfig.Mdt.SearchLimit) or 20, 50))
    ...
    if #tokens == 1 and #query >= 2 then
        pcall(function()
            local vehicle = exports[PoliceConfig.VehiclesResource]:GetVehicleByPlate(query:gsub('%s+', ''):upper())
    ...
    for _, row in ipairs(rows or {}) do
        ...
        results[#results + 1] = {
            ...
            stars = tonumber(row.stars) or 0,
            wanted = row.wanted ~= nil and PoliceLegacyDbBoolean(row.wanted) or false,
...
lib.callback.register('cm-police:server:mdtCitizenProfile', function(src, characterId)
    local authorized = authorizedForMdt(src)
    if not authorized or not PoliceLegacyRateLimit(src, 'police_mdt_profile', 600) then return nil end
    ...
    local vehicles = {}
    pcall(function() vehicles = exports[PoliceConfig.VehiclesResource]:GetVehiclesByOwner(characterId) or {} end)
    ...
    for _, row in ipairs(notes) do
        noteList[#noteList + 1] = {
            ...
            authorName = row.author_cid and PoliceLegacyNameFor(row.author_cid) or 'System',
    ...
    local licenses = {}
    for _, licenseType in ipairs(PoliceConfig.Mdt.LicenseTypes) do
    ...
    return {
        ...
        name = PoliceLegacyNameFor(characterId),
        citations = citations,
        bookings = bookings,
        impounds = impounds,
        ...
        wanted = status and PoliceLegacyDbBoolean(status.wanted) or false,
...
lib.callback.register('cm-police:server:mdtSetLicenseStatus', function(src, characterId, licenseType, status, reason)
    ...
    if not PoliceLegacyRateLimit(src, 'police_mdt_license', 800) then return false, 'Please wait.' end
    characterId = tostring(characterId or '')
    local validType = false
    for _, t in ipairs(PoliceConfig.Mdt.LicenseTypes) do if t == licenseType then validType = true break end end
...
lib.callback.register('cm-police:server:mdtSetCriminalStatus', function(src, characterId, stars, wanted, reason)
    local authorized, actorCid = authorizedForMdt(src)
    if not authorized then return false, 'You must be an on-duty officer with MDT permission.' end
    if not PoliceLegacyRateLimit(src, 'police_mdt_status', 800) then return false, 'Please wait.' end
    characterId = tostring(characterId or '')
    ...
    if onlineSrc then
        pcall(function() exports[PoliceConfig.PlayerDataResource]:SetWantedStars(onlineSrc, wanted and stars or 0) end)
    end
...
exports('SyncWantedStars', function(characterId, stars)
    if GetInvokingResource() ~= PoliceConfig.PlayerDataResource then return false end
    characterId = tostring(characterId or '')
    if characterId == '' then return false end
...
lib.callback.register('cm-police:server:mdtSetPhoto', function(src, characterId, url)
    local authorized, actorCid = authorizedForMdt(src)
    if not authorized then return false, 'You must be an on-duty officer with MDT permission.' end
    if not PoliceLegacyRateLimit(src, 'police_mdt_photo', 1000) then return false, 'Please wait.' end
    characterId = tostring(characterId or '')
    if characterId == '' then return false, 'Invalid character.' end
...
local function issueSingleFine(actorCid, characterId, violation)
    ...
    local removed = false
    if onlineSrc then
        removed = exports[PoliceConfig.PlayerDataResource]:RemoveMoney(onlineSrc, 'bank', violation.fine, 'police_citation', { violation = violation.id })
    else
    ...
    if not removed then
        FinishPoliceOperation(operationId, 'refunded', { reason = 'insufficient_funds', moneyRemoved = false })
        return false, ('%s does not have enough money in their bank account.'):format(PoliceLegacyNameFor(characterId))
    end
    ...
    if not called or committed ~= true then
        local refunded = false
        if onlineSrc then
            refunded = exports[PoliceConfig.PlayerDataResource]:AddMoney(onlineSrc, 'bank', violation.fine, 'police_citation_refund', { violation = violation.id }) == true
        else
...
lib.callback.register('cm-police:server:mdtIssueFine', function(src, characterId, violationId)
    ...
    if not PoliceLegacyRateLimit(src, 'police_mdt_fine', 1000) then return false, 'Please wait.' end
    characterId = tostring(characterId or '')
    ...
    return true, ('Issued a $%d citation to %s for %s.'):format(violation.fine, PoliceLegacyNameFor(characterId), violation.label)
end)
...
lib.callback.register('cm-police:server:mdtIssueFines', function(src, characterId, violationIds)
    ...
    for _, violation in ipairs(violations) do
        ...
        if not ok then
            ...
            return true, ('Issued %d of %d charges ($%d total) before %s ran out of funds.'):format(issuedCount, #violations, totalFine, PoliceLegacyNameFor(characterId))
        end
        issuedCount = issuedCount + 1
        totalFine = totalFine + feeOrMessage
    end
    return true, ('Issued %d charges to %s totaling $%d.'):format(issuedCount, PoliceLegacyNameFor(characterId), totalFine)
end)
...
lib.callback.register('cm-police:server:mdtVehicleSearch', function(src, plate)
    ...
    return {
        ...
        ownerCid = ownerCid,
        ownerName = ownerCid and PoliceLegacyNameFor(ownerCid) or 'Unknown',
...
lib.callback.register('cm-police:server:mdtIssueVehicleLicense', function(src, plate)
    local authorized, actorCid = authorizedForMdt(src)
    if not authorized then return false, 'You must be an on-duty officer with MDT permission.' end
    if not PoliceLegacyRateLimit(src, 'police_mdt_vehicle_license', 1000) then return false, 'Please wait.' end
    ...
    pcall(function() ok, message, licenseNumber = exports[PoliceConfig.VehiclesResource]:IssueVehicleLicense(plate) end)
...
lib.callback.register('cm-police:server:mdtAddNote', function(src, characterId, note)
    local authorized, actorCid = authorizedForMdt(src)
    if not authorized then return false, 'You must be an on-duty officer with MDT permission.' end
    if not PoliceLegacyRateLimit(src, 'police_mdt_note', 1000) then return false, 'Please wait.' end
    characterId = tostring(characterId or '')
    local clean = tostring(note or ''):gsub('[%c]', ' '):gsub('^%s+', ''):gsub('%s+$', '')
    if characterId == '' or clean == '' then return false, 'Invalid note.' end
    local maxLen = tonumber(PoliceConfig.Mdt.NoteMaxLength) or 500
...
lib.callback.register('cm-police:server:mdtAddEvidence', function(src, characterId, url, caption)
    local authorized, actorCid = authorizedForMdt(src)
    if not authorized then return false, 'You must be an on-duty officer with MDT permission.' end
    if not PoliceLegacyRateLimit(src, 'police_mdt_evidence', 1000) then return false, 'Please wait.' end
    characterId = tostring(characterId or '')
    url = tostring(url or ''):gsub('[%c]', ''):gsub('^%s+', ''):gsub('%s+$', '')
...
local function publicBolo(row)
    return {
        ...
        plate = row.plate,
        officerName = PoliceLegacyNameFor(row.officer_cid),
        createdAt = tostring(row.created_at or ''),
        clearedByName = row.cleared_by and PoliceLegacyNameFor(row.cleared_by) or nil,
...
lib.callback.register('cm-police:server:mdtFileUseOfForce', function(src, subject, forceType, narrative)
    local authorized, actorCid = authorizedForMdt(src)
    if not authorized then return false, 'You must be an on-duty officer with MDT permission.' end
    if not PoliceLegacyRateLimit(src, 'police_mdt_uof', 1500) then return false, 'Please wait.' end
    local validType = false
    for _, t in ipairs(PoliceConfig.Mdt.UseOfForceTypes) do if t == forceType then validType = true break end end
...
CreateThread(function()
    -- note's width must stay >= PoliceConfig.Mdt.NoteMaxLength (shared/config.lua)
