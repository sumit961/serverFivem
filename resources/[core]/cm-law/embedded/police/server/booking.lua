...
--
-- PoliceLegacyClearAll (server/cuffs.lua, a bare global) is called on a successful
...
local BookingRules = {
    minutesPerStar = tonumber(PoliceConfig.Booking.MinutesPerWantedStar) or 15,
    bookingRadius = tonumber(PoliceConfig.Booking.BookingRadius) or 8.0,
    handoffTimeoutMs = tonumber(PoliceConfig.Booking.HandoffTimeoutMs) or 10000,
}
...
local function sharedPrisonConfig()
    if GetResourceState('cm-prison') == 'started' then
        ...
        if ok and type(configured) == 'table' and type(configured.spawns) == 'table' then
            return configured.spawns, configured.release, configured.intake
        end
    end
    ...
    return JailSpawns, JailLocation, JailLocation
end
...
local function captureBookingMugshot(targetSrc, targetCid, bookingId)
    ...
    local absolutePath = ('%s/html/police/img/mugshots/%s'):format(GetResourcePath(GetCurrentResourceName()), filename)
...
local function bookingAuthority(src, targetSrc)
    ...
    if not actor or PoliceLegacyDbBoolean(actor.is_suspended) or not PoliceLegacyDbBoolean(actor.on_duty) or not has(actor, 'police.book') then
    ...
    local sharedSpawns, sharedRelease, intakeLocation = sharedPrisonConfig()
    ...
    if type(sharedRelease) ~= 'table' then return nil, 'The shared jail release point has not been configured.' end
    ...
    if stars < 1 or not PoliceLegacyDbBoolean(status and status.wanted) then
...
lib.callback.register('cm-police:server:bookingPreview', function(src, targetSrc)
    ...
    return true, { stars = booking.stars, minutes = booking.minutes, reason = booking.reason,
        suspectName = PoliceLegacyNameFor(booking.targetCid), characterId = booking.targetCid,
        officerName = PoliceLegacyNameFor(booking.actorCid), operationToken = token }
end)
...
local function policeAdmin(src)
    local ok, allowed = pcall(function()
        return exports[PoliceConfig.AdminResource]:HasPermission(src, PoliceConfig.AdminPermission)
    end)
...
function GetPoliceJailIntake() local _, _, intake = sharedPrisonConfig(); return locationPayload(intake) end
...
exports('GetJailIntake', GetPoliceJailIntake)
...
lib.callback.register('cm-police:server:jailNpcLocation', function()
    return GetPoliceJailIntake()
end)

local function prisonAreaWarning(coords)
    local area = PoliceConfig.Booking.PrisonArea
...
lib.callback.register('cm-police:server:adminConfig', function(src)
    ...
    local sharedSpawns, _, sharedIntake = sharedPrisonConfig()
    return {
        bookingDesk = locationPayload(BookingLocation), jailIntake = locationPayload(sharedIntake), jailSpawns = sharedSpawns, serviceNpc = locationPayload(ServiceNpcLocation),
...
lib.callback.register('cm-police:server:setAdminLocation', function(src, locationType, supplied)
    if not policeAdmin(src) or not PoliceLegacyRateLimit(src, 'police_admin_location', 800) then return false, 'Permission denied.' end
    ...
    if locationType == 'jail_intake' and GetResourceState('cm-prison') == 'started' then
        local ok, message, cfg = exports['cm-prison']:SetLocation(tonumber(src), 'intake')
        return ok, message, cfg and locationPayload(cfg.intake) or nil
    end
...
lib.callback.register('cm-police:server:resetAdminLocation', function(src, locationType)
    if not policeAdmin(src) or not PoliceLegacyRateLimit(src, 'police_admin_location_reset', 800) then return false, 'Permission denied.' end
    if locationType ~= 'booking_desk' and locationType ~= 'jail_intake' and locationType ~= 'jail_spawns' and locationType ~= 'service_npc'
        and locationType ~= 'armory_npc' and locationType ~= 'storage_npc' then return false, 'Invalid location type.' end
    if locationType == 'jail_intake' and GetResourceState('cm-prison') == 'started' then
        return exports['cm-prison']:ResetLocation(tonumber(src), 'intake')
    end
...
exports('PoliceLegacyAdminSetFacility', function(src, _, facilityType, reset)
    ...
    if not policeAdmin(src) or not PoliceLegacyRateLimit(src, 'police_admin_facility_export', 800) then return false, 'Permission denied.' end
    if locationType == 'jail_intake' and GetResourceState('cm-prison') == 'started' then
        if reset == true then return exports['cm-prison']:ResetLocation(tonumber(src), 'intake') end
        return exports['cm-prison']:SetLocation(tonumber(src), 'intake')
    end
    if reset == true then
...
lib.callback.register('cm-police:server:teleportAdminLocation', function(src, locationType)
    if not policeAdmin(src) or not PoliceLegacyRateLimit(src, 'police_admin_location_teleport', 1500) then return false, 'Permission denied.' end
    local facilityLocation = type(GetPoliceFacilityNpcLocation) == 'function' and GetPoliceFacilityNpcLocation(locationType) or nil
    local centralIntake = locationType == 'jail_intake' and GetPoliceJailIntake() or nil
    local location = locationType == 'booking_desk' and BookingLocation or locationType == 'jail_intake' and centralIntake
...
lib.callback.register('cm-police:server:adminReleaseCustody', function(src, targetCid)
    if not policeAdmin(src) or not PoliceLegacyRateLimit(src, 'police_admin_release_custody', 1000) then return false, 'Permission denied.' end
    ...
    if targetSrc then PoliceLegacyClearAll(targetSrc, true) end
    local actorCid = tostring(cid(src) or 'admin')
...
lib.callback.register('cm-police:server:saveAdminRules', function(src, payload)
    if not policeAdmin(src) or not PoliceLegacyRateLimit(src, 'police_admin_rules', 800) then return false, 'Permission denied.' end
...
lib.callback.register('cm-police:server:adminPrisoners', function(src)
    ...
    for _, row in ipairs(rows) do row.character_name = PoliceLegacyNameFor(row.character_id) end
    return rows
end)

lib.callback.register('cm-police:server:adminPrisonAction', function(src, action, characterId, minutes)
    if not policeAdmin(src) or not PoliceLegacyRateLimit(src, 'police_admin_prison_action', 900) then return false, 'Permission denied.' end
...
RegisterCommand('setpolicebooking', function(src)
    ...
    local actorCid = cid(src)
    local actor = actorCid and PoliceLegacyMemberFor(actorCid)
    if not actor or PoliceLegacyDbBoolean(actor.is_suspended) or not PoliceLegacyDbBoolean(actor.on_duty)
...
AddEventHandler('cm-police:server:bookingAction', function(src, targetSrc, action, payload, context)
    ...
    if not PoliceLegacyRateLimit(src, 'police_booking_action', 700) then return bookingNotify(src, 'Please wait.', 'error') end
    ...
    local bridgeOk, bridgeError = pcall(function() jailed, jailFailure = exports['cm-prison']:JailSuspect(src, targetSrc, booking.minutes, BookingRules.handoffTimeoutMs, {
        spawns = sharedSpawns, release = sharedRelease, reason = reason, arrestedBy = PoliceLegacyNameFor(booking.actorCid),
    }) end)
    if not jailed then
    ...
    pcall(function() exports[PoliceConfig.PlayerDataResource]:ClearWantedStars(targetSrc) end)
    PoliceLegacyClearAll(targetSrc, true)
...
local function reconcileBookings()
    ...
    for _, row in ipairs(rows) do
        ...
        local onlineSrc = PoliceLegacySourceFor(characterId)
        ...
        if (row.status == 'processing' or row.status == 'cuffed') and bookingId then
            if prisoner == true then
                ...
                if onlineSrc then
                    pcall(function() exports[PoliceConfig.PlayerDataResource]:ClearWantedStars(onlineSrc) end)
                    PoliceLegacyClearAll(onlineSrc, true)
                end
