...
--
--   cm_police_activity
--       The dashboard only ever reads the newest PoliceConfig.LogLimit rows, so
...
--       regardless of age.
--
--   cm_police_evidence + html/police/img/bodycam/*.jpg
...
local LOG_RETENTION_DAYS = math.max(1, math.floor(tonumber(PoliceConfig.LogRetentionDays) or 90))
local INCIDENT_RETENTION_DAYS = math.max(1, math.floor(tonumber(PoliceConfig.IncidentRetentionDays) or 30))
local SWEEP_INTERVAL_MS = math.max(600000, math.floor(tonumber(PoliceConfig.RetentionSweepMs) or 21600000))
local EVIDENCE_RETENTION_DAYS = math.max(1, math.floor(tonumber(PoliceConfig.EvidenceRetentionDays) or 30))
...
local function localCapturePath(url)
    ...
    return ('%s/html/police/%s'):format(resourcePath, url)
end
