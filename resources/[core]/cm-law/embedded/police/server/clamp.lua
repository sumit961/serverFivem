...
lib.callback.register('cm-police:server:toggleClamp', function(src, netId)
    ...
    if not PoliceLegacyRateLimit(src, 'police_toggle_clamp', 1000) then return false, 'Please wait.' end
    ...
    if #(GetEntityCoords(officerPed) - GetEntityCoords(vehicle)) > (PoliceConfig.Clamp.MaxDistance or 3.0) then
