...
local function resolveNearbyVehicle(src, netId)
    ...
    local row = exports[PoliceConfig.VehiclesResource]:GetVehicleByPlate(plate)
...
lib.callback.register('cm-police:server:impoundVehicle', function(src, netId)
    ...
    if not row then
        ...
        return true, 'Untracked vehicle towed.', {
            plate = 'UNREGISTERED', fee = 0, officerName = PoliceLegacyNameFor(actorCid),
    ...
    local impoundId = MySQL.insert.await([[INSERT INTO cm_police_impounds
        ...
        { row.id, organizationId or 'police', row.plate, ownerCid, actorCid, PoliceConfig.Impound.Fee })
    if session.evidenceId then
    ...
    TowSessions[src] = nil
    enforcementLog(src, actorCid, 'vehicle_impounded', { vehicleId = row.id, plate = row.plate, ownerCid = ownerCid, fee = PoliceConfig.Impound.Fee, organizationId = organizationId })
