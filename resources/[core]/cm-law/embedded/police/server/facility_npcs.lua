...
function IsNearPoliceFacilityNpc(src, locationType)
    ...
    return #(GetEntityCoords(ped) - vector3(location.x, location.y, location.z)) <= (PoliceConfig.FacilityNpcs.InteractDistance or 2.5) + 1.0
end
...
local function onDutyMember(src)
    ...
    if not member or PoliceLegacyDbBoolean(member.is_suspended) or not PoliceLegacyDbBoolean(member.on_duty) then return nil, characterId end
...
lib.callback.register('cm-police:server:openPoliceStorage', function(src)
    ...
    if not PoliceLegacyRateLimit(src, 'police_storage_open', 800) then return false, 'Please wait.' end
    ...
    local ok, opened, reason = pcall(function()
        return exports['cm-inventory']:OpenExternalInventory(src, {
            ...
            slots = math.max(1, math.min(30, tonumber(PoliceConfig.FacilityNpcs.StorageSlots) or 30)),
