...
local function manager(src)
    ...
    local admin = false
    pcall(function() admin = exports[PoliceConfig.AdminResource]:HasPermission(src, PoliceConfig.AdminPermission) == true end)
    if admin then return true, characterId end
    if not member or PoliceLegacyDbBoolean(member.is_suspended) or not has(member, 'police.manage_barricades') then return false, characterId end
...
CreateThread(function()
    ...
    if tonumber(MySQL.scalar.await('SELECT COUNT(*) FROM cm_police_barricade_catalog') or 0) == 0 then
        for _, model in ipairs(PoliceConfig.Barricades.DefaultModels or {}) do
