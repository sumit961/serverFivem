...
-- up to PoliceConfig.Wardrobe.MaxQuickSlots pointers any real member can set for
...
local function nearWardrobeNpc(ped)
    ...
    return #(GetEntityCoords(ped) - vector3(NpcLocation.x, NpcLocation.y, NpcLocation.z)) <= (PoliceConfig.Wardrobe.NpcInteractDistance or 2.5)
end
...
local function wardrobeOfficer(src)
    ...
    local ped = GetPlayerPed(src)
    if not member or PoliceLegacyDbBoolean(member.is_suspended) then return nil, characterId, 'You are not an active Police member.' end
...
lib.callback.register('cm-police:server:wardrobeClosetData', function(src)
    ...
    return { favorites = favoriteRows(characterId), maxSlots = PoliceConfig.Wardrobe.MaxQuickSlots or 5 }
end)
...
lib.callback.register('cm-police:server:finishWardrobeDuty', function(src, outfit, requestedSex)
    if not PoliceLegacyRateLimit(src, 'police_finish_wardrobe_duty', 1000) then return false, 'Please wait.' end
    local member, characterId, reason = wardrobeOfficer(src)
...
lib.callback.register('cm-police:server:saveWardrobeFavorite', function(src, name, outfit)
    ...
    if #encoded > 16000 then return false, 'Outfit data is too large.' end
    local maxSlots = PoliceConfig.Wardrobe.MaxQuickSlots or 5
...
lib.callback.register('cm-police:server:removeWardrobeFavorite', function(src, slot)
    ...
    if not member then return false, reason end
    slot = tonumber(slot)
    if not slot or slot < 1 or slot > (PoliceConfig.Wardrobe.MaxQuickSlots or 5) then return false, 'Invalid favorite slot.' end
...
lib.callback.register('cm-police:server:wardrobeMenuData', function(src, sex)
    local characterId = cid(src)
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or PoliceLegacyDbBoolean(member.is_suspended) then return nil end
...
lib.callback.register('cm-police:server:setWardrobeSlot', function(src, slot, presetId)
    local characterId = cid(src)
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or PoliceLegacyDbBoolean(member.is_suspended) then return false, 'You are not a Police member.' end
    slot = tonumber(slot)
    if not slot or slot < 1 or slot > (PoliceConfig.Wardrobe.MaxQuickSlots or 5) then return false, 'Invalid slot.' end
...
lib.callback.register('cm-police:server:wearOutfit', function(src, presetId, sex)
    if not PoliceLegacyRateLimit(src, 'police_wear_outfit', 800) then return false, 'Please wait.' end
    local characterId = cid(src)
    local member = characterId and PoliceLegacyMemberFor(characterId)
    if not member or PoliceLegacyDbBoolean(member.is_suspended) then return false, 'You are not a Police member.' end
