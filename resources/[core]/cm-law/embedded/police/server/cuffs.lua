...
local function authorized(src)
    ...
    if not member or PoliceLegacyDbBoolean(member.is_suspended) or not PoliceLegacyDbBoolean(member.on_duty) or not has(member, 'police.cuff') then
...
function PoliceLegacyClearAll(targetSrc, preserveCustody)
...
local function captureBodycamEvidence(src, actorCid, targetCid)
    ...
    if officerPed and officerPed ~= 0 then
        ...
        fallbackNote = ('BODY-CAM LOG: Cuffed by %s near %.0f, %.0f'):format(PoliceLegacyNameFor(actorCid), c.x, c.y)
    end
...
AddEventHandler('cm-police:server:cuffAction', function(src, targetSrc, action, payload, context)
    ...
    if not PoliceLegacyRateLimit(src, 'police_cuff_action', 700) then return cuffNotify(src, 'Please wait.', 'error') end
    ...
    if action == 'police_cuff' then
        ...
        -- editable text (PoliceConfig.Miranda), separate toast from the cuff
        -- notice above so it reads as its own moment.
        if PoliceConfig.Miranda and PoliceConfig.Miranda.Text then
            cuffNotify(targetSrc, PoliceConfig.Miranda.Text, 'inform')
        end
    elseif action == 'police_uncuff' then
        if not Cuffed[targetSrc] then return cuffNotify(src, 'That player is not cuffed.', 'error') end
        PoliceLegacyClearAll(targetSrc)
    ...
    elseif action == 'police_put_in_vehicle' then
        ...
        if requestedNetId and requestedNetId > 0 then
            ...
            if requested and requested ~= 0 and DoesEntityExist(requested) and GetEntityType(requested) == 2
                ...
                and #(GetEntityCoords(officerPed) - GetEntityCoords(requested)) <= (PoliceConfig.Cuffs.VehicleSeatDistance or 6.0) then
                vehicle = requested
            end
            ...
            vehicle = nearbyFreeSeatVehicle(
                ...
                PoliceConfig.Cuffs.VehicleSeatDistance or 6.0,
    ...
    elseif action == 'police_take_out_vehicle' then
        ...
        if #(GetEntityCoords(officerPed) - GetEntityCoords(vehicle)) > (PoliceConfig.Cuffs.VehicleSeatDistance or 6.0) then
...
RegisterNetEvent('cm-police:server:vehicleSeatResult', function(token, clientSucceeded)
    ...
    local valid = clientSucceeded == true and Cuffed[targetSrc] ~= nil
        ...
        and #(GetEntityCoords(officerPed) - GetEntityCoords(vehicle)) <= (PoliceConfig.Cuffs.VehicleSeatDistance or 6.0) + 2.0
...
local function restoreCustody(src)
    ...
    Cuffed[src] = { officerSrc = officerCid and PoliceLegacySourceFor(officerCid) or nil, officerCid = officerCid, targetCid = tostring(targetCid) }
