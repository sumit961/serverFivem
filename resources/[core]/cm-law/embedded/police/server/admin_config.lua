local NPCS = {
    front_desk = { label='Front Desk', facility='front_desk', key='service_npc', model=PoliceConfig.ServiceNpc.Model, name=PoliceConfig.ServiceNpc.Name, role=PoliceConfig.ServiceNpc.Role },
    armory = { label='Armory', facility='armory', key='armory_npc', model=PoliceConfig.FacilityNpcs.ArmoryModel, name=PoliceConfig.FacilityNpcs.ArmoryName, role=PoliceConfig.FacilityNpcs.ArmoryRole },
    storage = { label='Storage / Evidence', facility='storage', key='storage_npc', model=PoliceConfig.FacilityNpcs.StorageModel, name=PoliceConfig.FacilityNpcs.StorageName, role=PoliceConfig.FacilityNpcs.StorageRole },
    prison_intake = { label='Prison Intake', facility='intake', key='jail_intake', model='s_m_m_prisguard_01', name=PoliceConfig.JailNpc.Name, role=PoliceConfig.JailNpc.Role },
    wardrobe = { label='Wardrobe', key='wardrobe_npc', model=PoliceConfig.Wardrobe.NpcModel, name=PoliceConfig.Wardrobe.NpcName, role=PoliceConfig.Wardrobe.NpcRole },
    impound = { label='Impound Operator', key='impound_kiosks', model=PoliceConfig.Impound.OperatorModel, name=PoliceConfig.Impound.OperatorName, role=PoliceConfig.Impound.OperatorRole },
}
...
lib.callback.register('cm-police:server:capabilities',function(src)
    ...
    if not characterId or not PoliceLegacyMemberFor(characterId) then return {} end
    local values={}; for _,id in ipairs(CAPABILITIES) do values[id]=PoliceCapabilityEnabled(id) end
    return values
end)
exports('PoliceLegacyAdminGetCapabilities',function(src)
...
exports('PoliceLegacyAdminConfigureCapability',function(src,_,id,enabled)
...
allowed = function(src)
    local ok, value = pcall(function() return exports[PoliceConfig.AdminResource]:HasPermission(src, PoliceConfig.AdminPermission) end)
...
exports('PoliceLegacyAdminGetNpcs', function(src)
    if not allowed(tonumber(src)) then return { ok=false, error='Permission denied.' } end
...
exports('PoliceLegacyAdminConfigureNpc', function(src, _, data)
    ...
    if operation == 'reset' then
        ...
        if def.facility then exports[GetCurrentResourceName()]:PoliceLegacyAdminSetFacility(src, 'police', def.facility, true) end
    ...
    if operation == 'set_location' then
        if def.facility then
            local ok, message = exports[GetCurrentResourceName()]:PoliceLegacyAdminSetFacility(src, 'police', def.facility, false)
