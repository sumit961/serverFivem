...
local function searchAuthority(src, targetSrc)
    ...
    if not actor or PoliceLegacyDbBoolean(actor.is_suspended) or not PoliceLegacyDbBoolean(actor.on_duty) or not has(actor, 'police.cuff') then
...
lib.callback.register('cm-police:server:searchPlayer', function(src, targetSrc)
    if not PoliceLegacyRateLimit(src, 'police_search_player', 1000) then return nil, 'Please wait.' end
