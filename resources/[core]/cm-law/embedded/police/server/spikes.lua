...
local function authorizedOfficer(src)
    ...
    if member and not PoliceLegacyDbBoolean(member.is_suspended) and PoliceLegacyDbBoolean(member.on_duty) and has(member, 'police.spike') then
...
lib.callback.register('cm-police:server:deploySpikeStrip', function(src)
    if not PoliceLegacyRateLimit(src, 'police_deploy_spike', 1500) then return false, 'Please wait.' end
    ...
    if (OfficerCounts[actorCid] or 0) >= (PoliceConfig.SpikeStrips.MaxActive or 3) then
        return false, ('You already have the maximum of %d spike strips deployed.'):format(PoliceConfig.SpikeStrips.MaxActive or 3)
    end
    NextStripId = NextStripId + 1
    local stripId = NextStripId
    OfficerCounts[actorCid] = (OfficerCounts[actorCid] or 0) + 1
    -- Grace period matches PoliceConfig.SpikeStrips.PlacementTimeoutMs exactly --
    ...
    ActiveStrips[stripId] = { deploymentId = stripId, organizationId = orgId, netId = nil,
        ...
        expiresAt = os.time() + math.ceil((PoliceConfig.SpikeStrips.PlacementTimeoutMs or 45000) / 1000) }
    return true, stripId
end)
...
AddEventHandler('cm-police:server:spikeStripDeployed', function(stripId, x, y, z, heading)
    ...
    local entity = CreateObject(GetHashKey(PoliceConfig.SpikeStrips.Model), x, y, z, true, true, false)
    ...
    entry.netId = netId
    entry.expiresAt = os.time() + math.floor((PoliceConfig.SpikeStrips.LifetimeMs or 120000) / 1000)
