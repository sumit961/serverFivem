...
local function authorizedOfficer(src)
    ...
    if member and not PoliceLegacyDbBoolean(member.is_suspended) and PoliceLegacyDbBoolean(member.on_duty) and has(member, 'police.barricade') then
...
lib.callback.register('cm-police:server:deployBarricade', function(src)
    if not PoliceLegacyRateLimit(src, 'police_deploy_barricade', 1500) then return false, 'Please wait.' end
    ...
    if (OfficerCounts[actorCid] or 0) >= (PoliceConfig.Barricades.MaxActive or 2) then
        return false, ('You already have the maximum of %d barricades deployed.'):format(PoliceConfig.Barricades.MaxActive or 2)
    end
    NextBarricadeId = NextBarricadeId + 1
    local barricadeId = NextBarricadeId
    OfficerCounts[actorCid] = (OfficerCounts[actorCid] or 0) + 1
    -- Grace period matches PoliceConfig.Barricades.PlacementTimeoutMs exactly --
    ...
    ActiveBarricades[barricadeId] = { deploymentId = barricadeId, organizationId = orgId, netId = nil,
        ...
        expiresAt = os.time() + math.ceil((PoliceConfig.Barricades.PlacementTimeoutMs or 45000) / 1000) }
...
AddEventHandler('cm-police:server:barricadeDeployed', function(barricadeId, x, y, z, heading, modelName)
    ...
    if ped == 0 or #(GetEntityCoords(ped) - vector3(x, y, z)) > ((PoliceConfig.Placement.MaxDistance or 5.0) + 2.0) then return end
    ...
    entry.netId = netId
    entry.expiresAt = os.time() + math.floor((PoliceConfig.Barricades.LifetimeMs or 1800000) / 1000)
...
local function authorizedManager(src)
    ...
    if not member or PoliceLegacyDbBoolean(member.is_suspended) or not has(member, 'police.manage_barricades') then
        return nil, characterId
    end
...
-- PoliceConfig.SpikeStrips.Model today, which also isn't validated beyond the
...
CreateThread(function()
    ...
    -- as PoliceConfig.Ranks' own seeding (server/main.lua's setupDatabase()).
    ...
    if tonumber(existingCount) == 0 then
        for _, modelName in ipairs(PoliceConfig.Barricades.DefaultModels or {}) do
