...
window.cmRecallOrgFleet = async (orgId, model) => { const ok=window.CMUI&&await window.CMUI.confirm({title:'RECALL FLEET VEHICLE?',message:`Return ${model} for ${orgId} to its saved location? Occupied vehicles are refused.`,confirmText:'RECALL',cancelText:'CANCEL',dismissOnBackdrop:false}); if(ok)action('orgsRecallFleetVehicle',{orgId,model}); };
window.cmRecallAllOrgFleet = async (orgId) => { const ok=window.CMUI&&await window.CMUI.confirm({title:'RECALL ORGANIZATION FLEET?',message:`Return each enabled ${orgId} fleet vehicle to its own saved parking location? Occupied vehicles will be refused.`,confirmText:'RECALL ALL',cancelText:'CANCEL',dismissOnBackdrop:false}); if(ok)action('orgsRecallAllFleetVehicles',{orgId}); };
window.cmTuneOrgFleet = (orgId,model) => action('orgsTuneFleetVehicle',{orgId,model});
...
window.addEventListener('message', (event) => {
  ...
  if (msg.action === 'detailResult') {
    ...
    if (state.detail && state.detail.type === 'orgFleet') {
      ...
      card.innerHTML=`<h3>${esc(orgId)} Fleet</h3><p class="mini-label">Set / update location saves the position of the persistent fleet vehicle you are driving. Tune / Modify opens the existing Manage Vehicle editor; saved appearance is applied to the matching persistent fleet vehicle.</p><button class="btn" onclick="cmRecallAllOrgFleet('${esc(orgId)}')">Recall all enabled vehicles</button><div class="table-wrap"><table><thead><tr><th>Vehicle</th><th>Configured</th><th>Enabled</th><th>Tier</th><th></th></tr></thead><tbody>${items.map(v=>{const k=`${orgId}_${v.model}`.replace(/[^a-zA-Z0-9_]/g,'_');return `<tr><td>${esc(v.label||v.model)}</td><td>${v.configured?'Yes':'No'}</td><td><input id="orgFleetEnabled_${k}" type="checkbox" ${v.enabled?'checked':''}></td><td><input id="orgFleetTier_${k}" class="input" type="number" min="0" value="${Number(v.minTier||0)}"></td><td><button class="btn small primary" onclick="cmSaveOrgFleet('${esc(orgId)}','${esc(v.model)}')">Save</button><button class="btn small" onclick="cmBeginOrgFleet('${esc(orgId)}','${esc(v.model)}')">Set / update location</button><button class="btn small" onclick="cmRecallOrgFleet('${esc(orgId)}','${esc(v.model)}')">Recall</button><button class="btn small" onclick="cmTuneOrgFleet('${esc(orgId)}','${esc(v.model)}')">Tune / Modify</button><button class="btn small danger" onclick="cmResetOrgFleet('${esc(orgId)}','${esc(v.model)}')">Reset location</button></td></tr>`}).join('')}</tbody></table></div>`; page.prepend(card); return;
    }
