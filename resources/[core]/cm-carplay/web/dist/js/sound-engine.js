...
// import/export); depends on the bare global NUI (js/nui.js, loaded before
// this file); exposes window.SoundEngine (self-initialized below), read by
// js/music.js for isPlaying/currentTime/duration.
//
...
(function () {
  // This codebase's nui.js declares `const NUI = ...` at the top level of a
  // classic script -- that's a bare global (usable directly as `NUI` from
  // any other classic script sharing this page), NOT a `window.NUI`
  // property (top-level const/let never attach to `window`, unlike var).
  // Reference the bare global directly instead of destructuring it off
  // `window.NUI`, which is always undefined here.
  const nui = NUI.post;
  ...
  function isYoutubeUrl(url) {
    ...
    return /youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/(?:embed|shorts|live)\//.test(url);
  }
  ...
  function normalizeSpeakerConfig(config) {
    ...
    return {
      ...
      outsideVolumeFactor: numberOr(config.outsideVolumeFactor, OUTSIDE_VEHICLE_VOLUME_FACTOR),
    };
  }
  ...
  class AudioManager {
    ...
    getYoutubeHost() {
      if (!this.youtubeHostEl) {
        ...
        // A zero-sized iframe is rejected by some CEF/YouTube builds. Keep a
        // real player surface off-screen and almost invisible so the direct
        // YouTube fallback can initialize reliably.
        this.youtubeHostEl.style.position = "fixed";
        ...
        this.youtubeHostEl.style.zIndex = "-999";
    ...
    createYoutubeSource(audioSourceId, url, config) {
      ...
      this.loadYoutubeApi().then((YT) => {
        ...
        entry.player = new YT.Player(container.id, {
          // Some CEF/YouTube builds refuse autoplay or never emit state
          // changes for a zero-by-zero iframe. The container is already
          // positioned off-screen and nearly transparent, so keep a valid
          // player surface without exposing it to the player.
          height: "200",
          width: "200",
          videoId,
          playerVars: {
    ...
    applySpeakerConfigToNodes(speaker, immediate) {
      ...
      const outsideFactor = config.pannerDisabled ? 1 : clamp(numberOr(config.outsideVolumeFactor, OUTSIDE_VEHICLE_VOLUME_FACTOR), 0, 1);
        ...
        if (sourceEntry.kind === "html5" && speaker.panner) {
          ...
          const gain = speaker.config.pannerDisabled
            ...
            : computeManualDistanceGain(speakerUpdate.distance, speaker.config) *
              ...
              clamp(numberOr(speaker.config.outsideVolumeFactor, OUTSIDE_VEHICLE_VOLUME_FACTOR), 0, 1);
    ...
    async handleFetchData(message) {
      ...
      try {
        ...
        // CEF may reject cross-origin metadata requests even when the video
        // itself is playable. A valid ID is enough to import the track; the
        // server resolver or the IFrame fallback will validate playback.
        const videoId = isYoutubeUrl(url) ? (extractYoutubeVideoId(url) || url) : null;
        payload = videoId
          ? { url, title: videoId, videoId, status: "success" }
          : {
              url,
  ...
  // js/music.js polls window.SoundEngine.getState(id) for read-only
  // isPlaying/currentTime/duration; self-initialize immediately (matching
  // this file's original convention) instead of requiring a `new` call
  // from app.js.
  window.SoundEngine = new AudioManager();
})();
