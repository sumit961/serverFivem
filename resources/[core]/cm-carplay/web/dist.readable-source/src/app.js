// cm-carplay web UI -- shell/state orchestrator. Classic script (no
// import/export); depends on window.NUI, window.CarPlayState,
// window.CarPlayViews and window.AudioManager (all loaded before this file).

(function () {
  const { listen, nui } = window.NUI;
  const { createState, updateState, clockLabel } = window.CarPlayState;
  const { render } = window.CarPlayViews;
  ...
  function repaint() {
    ...
    // client/main.lua's startDashcam creates a real full-screen game camera
    // (CreateCam + RenderScriptCams) -- the opaque tablet chrome has to get
    // out of the way while it's active or that live view is never visible.
    document.body.classList.toggle(
      "camera-live",
      state.activeTab === "camera" && state.camera.active,
    );
  }
  ...
  function handleMessage(message) {
    ...
    switch (message.action || message.type) {
      ...
      case "updatePlayerPosition": {
        // Fires every 5s while the menu is open. A full setState()/repaint()
        // here would tear down and rebuild the whole tablet just to update a
        // coordinate readout -- mutate state directly and patch only the
        // one label that's currently showing it.
        state.position = data || state.position;
        const coordLabel = document.getElementById("map-coordinates-label");
        if (coordLabel) {
          coordLabel.textContent = `${Number(state.position.x || 0).toFixed(2)}, ${Number(state.position.y || 0).toFixed(2)}`;
        }
        break;
      }
      case "updateGameTime": {
        // Fires every 1s while the menu is open -- same reasoning as
        // updatePlayerPosition above, this is what caused the whole tablet
        // to visibly flash/rebuild once a second.
        state.time = data || state.time;
        const clockEl = document.getElementById("clock-label");
        if (clockEl) clockEl.textContent = clockLabel(state.time);
        break;
      }
      case "updateWeather": {
        // Fires every 30s while the menu is open -- only repaint if the
        // reading actually moved.
        const next = data || {};
        const prev = state.weather || {};
        if (next.type !== prev.type || next.temperature !== prev.temperature) {
      ...
      case "updateVehiclePlate": {
        // Fires every 5s while the menu is open -- only repaint on an actual
        // vehicle change, not every poll of the same plate.
        const next = data || "—";
        if (next !== state.plate) setState({ plate: next });
        break;
      }
      case "updateWaypoint": {
        // Fires every 1s while the menu is open. getWaypointInfo() on the
        // Lua side includes a live distance-to-waypoint that changes every
        // tick while driving, but nothing in this UI displays it -- only
        // repaint when presence or the waypoint's coordinates actually
        // change, so sitting still (or driving toward a fixed waypoint)
        // doesn't rebuild the tablet once a second.
        const next = data || null;
        const prev = state.waypoint;
        const changed =
  ...
  async function handleAction(button) {
    ...
    if (action === "fetchYouTubeData") {
      ...
      if (result?.success) {
        ...
        // nui() swallows network-level failures and resolves null so the
        // browser preview keeps working -- surface that case too instead of
        // failing silently in-game.
        notice("Could not reach the server, try again");
      }
      return;
    }
    ...
    if (action === "toggleDoor") {
      ...
      payload = { door, open };
      const doors = (Array.isArray(state.vehicle.doors)
        ? state.vehicle.doors
        : [false, false, false, false]
      ).slice();
      doors[door] = open;
      setState({ vehicle: { doors } });
    }
    if (
      [
        "toggleEngine",
    ...
    if (action === "startDashcam") {
      ...
      if (!result || result.success !== true) {
        notice("You must be in a vehicle to use the dashcam");
        return result;
      }
