...
Config.Debug = false -- Enable only while diagnosing CarPlay on a test server

-- Gate CarPlay to specific vehicles instead of every car.
-- If true, a vehicle only has CarPlay if EITHER:
--   (a) its model is flagged "CarPlay" in rn-vehicleshop's Manage Vehicles, or
--   (b) the 'carplay' item has been used inside that specific vehicle.
-- If false, any player can open CarPlay in any vehicle using the keybind.
Config.RequireCarplayItem = true

Config.PrimaryColor = '#00D1FF' -- CM cyan accent used by the shared framework UI

-- Notification bridge -> cm-core (forwards to cm-hud toast, matches server-wide notify style)
Config.Notify = function(message, type)
    -- type: 'success', 'error', 'warning', 'info'
    local ok = pcall(function()
        exports['cm-core']:Notify(message, type or 'info')
    end)
    if ok then return end

    -- Fallback if cm-core isn't loaded for some reason.
    local prefix = type == 'success' and '^2' or type == 'error' and '^1' or '^3'
    TriggerEvent('chat:addMessage', { args = { prefix .. 'CarPlay', message } })
end

-- Seatbelt bridge -> cm-vehicles, the server's authoritative seatbelt owner.
-- cm-vehicles only exposes a toggle (no "set to state" command), so this flips
-- the real seatbelt via its keybind command rather than tracking our own state.
Config.ToggleSeatbelt = function(state)
    pcall(function() ExecuteCommand('cm_seatbelt') end)
end
...
-- Optional YouTube anti-bot authentication. Leave server/youtube-cookies.json
-- as [] for public videos. If YouTube challenges your server IP, export the
-- cookies from a separate YouTube account and place the JSON array in that
-- file, or set the cm_carplay_youtube_cookies convar before starting the
-- resource. Never commit personal cookies to a public repository.
