...
local function pruneActivity()
    local ok, deleted = pcall(function()
        return MySQL.update.await([[
            DELETE FROM cm_ems_activity WHERE created_at < DATE_SUB(NOW(), INTERVAL ? DAY)
        ]], { LOG_RETENTION_DAYS })
    end)
    if not ok then
        print(('[cm-ems] retention: activity sweep failed: %s'):format(tostring(deleted)))
        return 0
    end
...
local function pruneIncidents()
    local ok, deleted = pcall(function()
        return MySQL.update.await([[
            DELETE FROM cm_ems_incidents
            WHERE status IN ('resolved', 'expired', 'cancelled', 'closed')
              AND created_at < DATE_SUB(NOW(), INTERVAL ? DAY)
        ]], { INCIDENT_RETENTION_DAYS })
    end)
    if not ok then
        print(('[cm-ems] retention: incident sweep failed: %s'):format(tostring(deleted)))
        return 0
    end
    return tonumber(deleted) or 0
end
...
local function sweep()
    ...
    if logs > 0 then
        print(('[cm-ems] retention: removed %d activity rows older than %d days')
            :format(logs, LOG_RETENTION_DAYS))
    end

    local incidents = pruneIncidents()
    if incidents > 0 then
        print(('[cm-ems] retention: removed %d closed dispatch calls older than %d days')
...
RegisterCommand('emsretention', function(src)
    if src ~= 0 then
        print('[cm-ems] retention: console only.')
        return
    end
    sweep()
end, true)
