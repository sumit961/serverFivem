-- CM UI bridge: keeps ox_lib for callbacks only. Every player-facing prompt,
-- context and notification is rendered by CM NUI / cm-hud.
local pending = {}
local contexts = {}
local sequence = 0
...
local function nextId()
    sequence = sequence + 1
    return ('%s:%d'):format(GetCurrentResourceName(), sequence)
end

function CMNotify(payload, kind)
    ...
    if GetResourceState('cm-hud') == 'started' then
        ...
        TriggerEvent('chat:addMessage', { args = { 'Property', message } })
    end
end
...
function CMInputDialog(title, fields)
    return awaitUi('input', title, fields or {})
end

function CMAlertDialog(options)
    options = options or {}
...
function CMShowContext(id)
    ...
    SetNuiFocus(true, true)
    SendNUIMessage({ action = 'uiBridge:open', mode = 'context', id = tostring(id), title = context.title or 'Menu', content = rows })
end

function CMHideContext()
