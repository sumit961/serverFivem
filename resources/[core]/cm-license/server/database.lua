...
function Database.Init()
    ...
    local statements = {
        ...
        'CREATE TABLE IF NOT EXISTS cm_character_licenses (id INT PRIMARY KEY AUTO_INCREMENT, character_id INT NOT NULL, license_type_id INT NOT NULL, issued_at BIGINT NOT NULL, expires_at BIGINT NOT NULL, status ENUM(\'active\', \'expired\', \'revoked\') NOT NULL DEFAULT \'active\', revoked_at BIGINT, revoked_by INT, revoke_reason VARCHAR(255), created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY (license_type_id) REFERENCES cm_license_types(id) ON DELETE RESTRICT, UNIQUE KEY unique_character_license (character_id, license_type_id), INDEX idx_character_id (character_id), INDEX idx_license_type_id (license_type_id), INDEX idx_expires_at (expires_at), INDEX idx_status (status)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4',
        'CREATE TABLE IF NOT EXISTS cm_license_active_tests (id INT PRIMARY KEY AUTO_INCREMENT, character_id INT NOT NULL, license_type_id INT NOT NULL, test_started_at BIGINT NOT NULL, current_checkpoint INT NOT NULL DEFAULT 0, total_checkpoints INT NOT NULL, vehicle_netid INT, mistakes INT NOT NULL DEFAULT 0, max_mistakes INT NOT NULL, status ENUM(\'waiting_start\', \'in_progress\', \'completing\', \'completed\', \'failed\', \'cancelled\') DEFAULT \'waiting_start\', fail_reason VARCHAR(100), created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, FOREIGN KEY (license_type_id) REFERENCES cm_license_types(id) ON DELETE CASCADE, INDEX idx_character_id (character_id), INDEX idx_status (status)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4'
    }
    ...
    local npcCoords = json.encode({x=npc.Coords.x,y=npc.Coords.y,z=npc.Coords.z,heading=npc.Coords.heading,scenario=npc.Scenario})
    for licenseType, definition in pairs(CMLicenseConfig.StandardTypes or {}) do
        MySQL.query.await([[INSERT INTO cm_license_types
            ...
            npc_model=VALUES(npc_model),npc_coords=VALUES(npc_coords),enabled=true]],
            {licenseType,definition.label,definition.item,definition.price,definition.days,definition.model,
             definition.category,npc.Model,npcCoords})
    end
...
function Database.IssueLicense(characterId, licenseTypeId, validDays)
    ...
    return MySQL.query.await([[INSERT INTO cm_character_licenses
...
function Database.MarkLicenseDelivered(characterId, licenseTypeId)
    return MySQL.update.await([[UPDATE cm_character_licenses
...
function Database.UpdateTestSession(testId, updates)
    ...
    local allowed = { current_checkpoint = true, status = true, vehicle_netid = true,
        mistakes = true, fail_reason = true }
