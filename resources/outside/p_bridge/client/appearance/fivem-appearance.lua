if (Config.Appearance == 'auto' and not checkResource('fivem-appearance')) or (Config.Appearance ~= 'auto' and Config.Appearance ~= 'fivem-appearance') then
    return
end
...
if Config.Debug then
    lib.print.info('[Appearance] Loaded: fivem-appearance')
end
...
Bridge.Appearance.fetchDatabaseSkin = function()
    local databaseSkin = lib.callback.await('fivem-appearance:server:getAppearance', false)
...
Bridge.Appearance.setPlayerClothing = function(clothingData)
    ...
    exports['fivem-appearance']:setPedComponents(cache.ped, clothingData.components)
    exports['fivem-appearance']:setPedProps(cache.ped, clothingData.props)
